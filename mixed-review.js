...
function startAndEndWith(letter) {
    let sameBegAndEndLetters = [];
    for (let item of words) {
        if (item[0] == letter && item[item.length - 1] == letter) {
            yy.push(item);
        }
    }
    return sameBegAndEndLetters;
}
// These should all print true when you are done.
...
// console.log(countMoney(myCoins));
...
function waitFor24() {
    let answer;
    while (answer != 24) {
        answer = readline.question('enter a number: ');
    }
}
